...
class Login:
    def __init__(
        self, user_repo: UserRepoPort, password_port: PasswordPort, token_port: TokenPort
    ):
    ...
    def execute(self, input_data: LoginInput) -> LoginOutput:
        ...
        if not self._password_port.verify_password(input_data.password, user.password_hash):
            raise ValueError("Invalid credentials")
